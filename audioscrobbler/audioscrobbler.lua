...
local function sendHandshake()
  ...
  local token = mympd.hash_md5(mympd.hash_md5(mympd_env.var.scrobbler_password) .. timestamp)
  local data = {
    hs = "true",
    p = "1.2",
    c = "myMPD",
    v = "1.0",
    u = mympd_env.var.scrobbler_username,
    t = timestamp,
    a = token
  }
  local rc, code, headers, body = mympd.http_client("GET", mympd_env.var.scrobbler_handshake_uri .. "?" .. urlencode_data(data), "", "")
...
local scrobbler_session_id, playing_uri, submit_uri = sendHandshake()
...
if scrobbler_session_id == nil then
  return "libre.fm handshake failed."
end

mympd.log(7, "scrobbler_session_id: " .. scrobbler_session_id)
